...
-- Plugin state
M.enabled = true  -- Track if plugin is completely enabled/disabled
...
local function get_highlight_name(attrs)
  ...
  -- Note: dim attribute not supported by Neovim, skip it
  if attrs.italic then
    ...
    while col < line_len do
      ...
      if not attrs.reset then
        ...
        if text_start <= text_end then
          ...
          if hl_name then
            create_highlight_group(hl_name, attrs)
            -- Use a separate namespace for highlights to avoid interfering with concealment extmarks
            vim.api.nvim_buf_add_highlight(bufnr, hl_ns_id, hl_name, line_idx - 1, text_start - 1, text_end)
          end
        end
      end
      ...
      if config.conceal then
        ...
        local mark_line = line_idx  -- 1-indexed

        local should_conceal = true
        if config.reveal_under_cursor and mark_line == cursor_line then
...
local function setup_autocmds()
  ...
  vim.api.nvim_create_autocmd({
    ...
    callback = function()
      ...
      if config.conceal then
        ...
        vim.wo.concealcursor = config.reveal_under_cursor and "" or "nvic"
      end
  ...
  -- Add cursor movement handler for reveal_under_cursor
  vim.api.nvim_create_autocmd({ "CursorMoved", "CursorMovedI" }, {
    group = group,
    pattern = "*",
    callback = function()
      if not M.enabled or not config.conceal or not config.reveal_under_cursor then
        return
      end
      local filetype = vim.bo.filetype
      if not should_process_filetype(filetype) then
...
-- Toggle reveal under cursor option
function M.toggle_reveal()
  config.reveal_under_cursor = not config.reveal_under_cursor
