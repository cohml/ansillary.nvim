describe("ansillary integration tests", function()
  ...
  describe("concealment integration", function()
    it("should create extmarks for concealment", function()
      ...
      ansillary.setup({conceal = true, reveal_under_cursor = false})
      ansillary._trigger_highlighting_for_tests()
    ...
    it("should respect reveal_under_cursor setting", function()
      helper.create_test_buffer({"\\033[31mred text\\033[0m"})
      vim.api.nvim_win_get_cursor = function(winid) return {1, 0} end

      ansillary.setup({conceal = true, reveal_under_cursor = true})
      ansillary._trigger_highlighting_for_tests()
      ...
      assert.is_false(concealed_on_cursor_line, "ANSI codes concealed on cursor line when reveal_under_cursor is true")
    end)
  end)
  ...
  describe("full workflow integration", function()
    ...
    it("should handle toggle operations correctly", function()
      ...
      -- Toggle reveal under cursor
      local initial_reveal = ansillary.get_state().config.reveal_under_cursor
      ansillary.toggle_reveal()
      assert.are.equal(not initial_reveal, ansillary.get_state().config.reveal_under_cursor)
